...
export class credentialsServices {
  ...
  static async getCredentialsById(credentialsId: UUID) {
    try {
      const response = (await CredentialsModel.findByPk(credentialsId)) as
        | Promise<credentialModelInfo>
        | Record<string, any>;
      return response;
    } catch (error) {
      throw error;
  ...
  static async getCredentialsByMail(mail: string) {
    try {
      const response = (await CredentialsModel.findOne({
        where: { mail: mail },
      })) as Record<string, any> | Promise<credentialModelInfo>;
      return response;
    } catch (error) {
      throw error;
    }
  ...
  static async getAttemptDate(credentialsId: UUID) {
    try {
      const { failedAttemptsDate } = await this.getCredentialsById(
        credentialsId
      );
  ...
  static async adAttempt(credentialsId: UUID) {
    try {
      await CredentialsModel.increment("failedAttempts", {
  ...
  static async updateAuhtCredentials(
    credentialsId: UUID,
